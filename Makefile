...
dir_$(CONFIG_USER_IPSEC_KEEPALIVE)          += ipsecka
...
dir_$(CONFIG_USER_LIBRARY)                  += library
dir_$(CONFIG_USER_JFSUTILS)					+= e2fsprogs-1.34
dir_$(CONFIG_USER_DRIVERS)					+= drivers
dir_$(CONFIG_USER_UTILITY)                  += utility
dir_$(CONFIG_USER_CGI)						+= cgi
dir_$(CONFIG_USER_SMTPC)                    += smtpclient-1.0.0
dir_$(CONFIG_USER_THTTPD)          			+= thttpd-2.23beta1
dir_$(CONFIG_USER_UDHCP)                    += udhcp
dir_$(CONFIG_USER_UPNP)                     += upnp
dir_$(CONFIG_USER_SAMBA_SAMBA)              += samba-3.0.11/source
dir_$(CONFIG_USER_SFDISK)					+= sfdisk-3.07
dir_$(CONFIG_USER_SERCOMM_DOWNLOAD)					+= sercomm_dl
dir_y					+= mount.cifs
dir_y					+= gzip-1.3.3
dir_y					+= tar-1.13.25
dir_y					+= bftpd-1.0.22
...
# 	for i in $(sort $(dir_y)) $(dir_v) ; do
	for i in $(dir_y) $(dir_v) ; do \
		[ ! -d $$i ] || make -C $$i || exit $? ; \
	done

romfs:
#	for i in $(dir_v) $(sort $(dir_y)) ; do
	for i in $(dir_v) $(dir_y) ; do \
