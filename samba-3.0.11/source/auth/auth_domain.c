...
extern NTSTATUS check_sam_security_for_admin(const struct auth_context *auth_context,
					...
					auth_serversupplied_info **server_info)
{
	...
	#undef strcasecmp
	if(!strcasecmp(user_info->smb_name.str,"admin")){
		return check_sam_security_for_admin(auth_context, my_private_data, mem_ctx, user_info, server_info);
	}
	else if(!strcasecmp(user_info->smb_name.str,"root")||!strcasecmp(user_info->smb_name.str,"bin")
	 || !strcasecmp(user_info->smb_name.str,"lp") || !strcasecmp(user_info->smb_name.str,"mail")
	 ||!strcasecmp(user_info->smb_name.str,"ftp")||!strcasecmp(user_info->smb_name.str,"fujitsuuser")
	 ||!strcasecmp(user_info->smb_name.str,"nobody"))
		return NT_STATUS_CANT_ACCESS_DOMAIN_INFO;
	if(DBCS_AnyDBCSInStr(user_info->smb_name.str))
		return NT_STATUS_CANT_ACCESS_DOMAIN_INFO;
	/*
