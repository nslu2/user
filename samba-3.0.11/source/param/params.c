...
static BOOL Section( myFILE *InFile, BOOL (*sfunc)(const char *) )
  ...
  while( (EOF != c) && (c > 0) )
    ...
    if( i > (bSize - 2) )
	 ...
	 if(i>0){
		if(!j_second && ((c>=0x40 && c<=0x7E) || (c>=0x80 && c<=0xFC))){
			if((pre_char >= 0x80 && pre_char <= 0x9F) || (pre_char >= 0xE0 && pre_char <= 0xEF) || ( pre_char >= 0xF0 && pre_char <= 0xFC))
				j_second=1;
			else
				j_second=0;
    ...
    switch( c )
      ...
      //Ignore ']' in Japanese character.
		if (i>=1 && ((bufr[i-1] >= 0x80 && bufr[i-1] <= 0x9F) || (bufr[i-1] >= 0xE0 && bufr[i-1] <= 0xEF) || ( bufr[i-1] >= 0xF0 && bufr[i-1] <= 0xFC) ) && j_second){
			bufr[i++] = c;
			end = i;
			pre_char=c;
			c = mygetc( InFile );
