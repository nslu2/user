...
static void
add_partition(int n, int sys,unsigned int *partition_start,unsigned long *partition_stop) {
...
//					 0, mesg);
	...
	if ((cround(start) == cround(limit)) || (n==2) ) {
		stop = limit;
	} else {
...
static void
new_partition(int partition,unsigned int *partition_start,unsigned long *partition_stop) {
...
#define FDISKFILE "/var/tmp/fdisk"
int
...
//		disk_device=dev;
...

	{
		...
		while(fgets(buf,sizeof(buf),fp)){
			if(sscanf(buf,"%d,%ld,%d\n",&start,&cylinders,&type) != 3){
        		fclose(fp);
        		return -1;
    		}
    		stop=start+cylinders;
    		new_partition(i,&start,&stop);
    		i++;
    		if(i==3){
    			change_sysid(i-1);
				break;
